...
class BusinessReviewAnalyzer:
    ...
    def analyze(self, data, user_query):
        """
        ...
        report = {
            ...
            'example_comments_per_topic': {category: texts[:2] for category, texts in self.key_topics.items()}  # First 3 comments for each topic
        }
        print("finished analyzing report")
        return report
...
#     pass
    ...
    # with open('data/costa/costa_data.json', 'r') as f:
    #     data = json.load(f)
    ...
    # with open(f'data/{costa}/costa_result.json', 'w') as f:
    #     json.dump(result, f, indent = 4)

    # # convert json file to text file
    # with open('data/costa/costa_result.txt', 'w') as f:
